...
2. Install the **Keep GitHub Application** either through the onboarding screen or by visiting **[this link](https://github.com/apps/keephq)**. The installation process is straightforward and user-friendly.
   ![GitHub App Installation](/images/github-app-install.png)
...
<Accordion title="ngrok">
  ...
  **How to start Keep with ngrok**
  ...
  <Note>`USE_NGROK` is enabled by default when running with `docker-compose`</Note>
  ...
  **How to obtain ngrok URL?**
  ...
  <Note>You can check that the ngrok tunnel is working properly by sending a simple HTTP GET request to `/healthcheck` Try: `curl -v https://fab5-213-57-123-130.ngrok.io/healthcheck` in our example.</Note>
...
![Generated Example](/images/first-alert.yaml.png)
